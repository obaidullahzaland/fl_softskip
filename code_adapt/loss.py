...
class ZSGLoss(nn.Module):
    ...
    def __init__(self, ratios, scales, cfg):
        ...
        if self.use_att_loss:
            self.loss_keys = ['loss', 'cls_ls', 'box_ls','att_ls','gest_ls']
        else:
            self.loss_keys = ['loss', 'cls_ls', 'box_ls','gest_ls']
        self.anchs = None
    ...
    def forward(self, out: Dict[str, torch.tensor],
        ...
        gest_ls = self.gest_loss(gest,gt_gest)
        # + self.lamb_rel * rel_loss
        out_loss += gest_ls
        out_dict = {}
