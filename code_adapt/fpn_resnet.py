...
#         self.backbone = backbone
    ...
    def forward(self, inp):
        ...
        #print(p3_out.shape, p4_out.shape, p5_out.shape, p6_out.shape, p7_out.shape, p8_out.shape)
        return [p3_out, p4_out, p5_out, p6_out, p7_out, p8_out]



class FPN_prune_backbone(nn.Module):
    """
    A different fpn, doubt it will work
    """
...
#         self.backbone = backbone
    ...
    def forward(self, inp,weights):
        ...
        c3, c4, c5 = inp
        w3, w4, w5 = weights
        p51 = w5[:,0:256,:,:]*self.P5_1(c5) + c5[:,0:256,:,:]
        #p51 = w5*self.P5_1(c5) + c5
        p5_out = w5[:,0:256,:,:]*self.P5_2(p51) + p51

        # p5_up = F.interpolate(p51, scale_factor=2)
        p5_up = F.interpolate(p51, size=(c4.size(2), c4.size(3)))
        p41 = w4[:,0:256,:,:]*self.P4_1(c4) + p5_up
        p4_out = w4[:,0:256,:,:]*self.P4_2(p41) + p41

        # p4_up = F.interpolate(p41, scale_factor=2)
        p4_up = F.interpolate(p41, size=(c3.size(2), c3.size(3)))
        p31 = w3[:,0:256,:,:]*self.P3_1(c3) + p4_up
        p3_out = w3[:,0:256,:,:]*self.P3_2(p31) + p31
