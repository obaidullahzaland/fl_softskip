...
class Learner:
    ...
    def __post_init__(self):
        ...
        self.complexity = self.cfg['complexity']
    ...
    def validate(self, db: Optional[DataLoader] = None,
        ...
        with torch.no_grad():
            ...
            for batch in progress_bar(db, parent=mb):
                ...
                metric = self.eval_fn(out, batch,debug)
                for result,id_ in zip(metric['result'],metric['idxs']):
                    id_ = int(id_.item())
                    result = int(result.item())
                    out_df.loc[id_,"model_3"]=result
            ...
            print("Accuracy: ",self.eval_fn.get_accuracy())
            print("Latency: ", np.mean(times)," ",np.std(times))
            if self.complexity == True:
                out_df.to_csv('data/kitchen/csv_dir/val_comp.csv', index=False)
    ...
    def testing(self, db: Dict[str, DataLoader]):
        ...
        for dl_name, dl in tqdm(db.items(), total=len(db)):
            print("Testing")
            out_loss, out_acc, preds = self.validate(dl,debug=True)
